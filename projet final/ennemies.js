...
async function spawnEnemy(type) {
    ...
    try {
        const spriteSheet = await loadImage(config.spriteSheet);
...
// Joue une animation
function playAnimation(enemyId, animName, onComplete) {
...
function killEnemy(enemyId) {
    ...
    playAnimation(enemyId, deathAnim, () => {
        removeEnemy(enemyId);
    });
}
